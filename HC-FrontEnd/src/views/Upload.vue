<template>
  <div class="page2">
    <ImageUploader @file-selected="choose_way" />
...
const choose_way = (file) => {
  console.log(file);
  console.log("模型路径为", settingsStore.modelPath); // 修正这里的 console.log 调用
  if (settingsStore.modelPath) {
    console.log("使用模型进行预测");
    model_predict(file); // 传递文件给 model_predict
  } else {
    console.log("使用HSV阈值提取");
    uploadImage_hsv(file); // 传递文件给 uploadImage_hsv
  }
};
const model_predict = async (file) => {
  try {
    isWait.value = true;
    const formData = new FormData();
    formData.append('file', file); // 'file' 是请求的字段名
    imageStore.updateOriginalImage(URL.createObjectURL(file));
    //更新原始文件
    fileStore.updateOriginalFile(file);

    const response = await axios.post("http://localhost:5000/model_predict", formData, {
      responseType: 'blob'  // 设置响应类型为 blob
    });

    // 检查响应状态
    if (response.status === 200) {
      const blob = response.data;  // blob 数据直接在 response.data 中
      const imageUrl = URL.createObjectURL(blob); // 创建可用的 URL
      line_img.value = imageUrl; // 将 URL 赋值给图像组件
      imageStore.updateGrayscaleImage(imageUrl); // 更新图像显示
      showKImg();
      isWait.value = false;
    } else {
      console.error('模型预测时出错:', response.data);
    }
  } catch (error) {
    console.error('模型预测时出错:', error);
  }
};
const uploadImage_hsv = async (file) => {
  try {
    isWait.value = true;
    const formData = new FormData();
...
onMounted(() => {
  if (settingsStore.isSetUpdate) {
    ...
    uploadImage_hsv(fileStore.originalFile);
